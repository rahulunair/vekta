...
def run_vekta(
    command: str, input_data: str = None, top_k: int = None, search_method: str = None
) -> str:
    env = os.environ.copy()
...
def parse_search_results(output: str):
    try:
        ...
        return {
            ...
            "timings": {"search_duration_ms": 0, "sort_duration_ms": 0},
        }
...
def perform_search(embedding, top_k: int, search_method: str):
    try:
        ...
        search_result = run_vekta(
            "search", json.dumps(embedding), top_k=top_k, search_method=search_method
        )
        end_time = time.time()
...
def main():
    ...
    try:
        ...
        all_results = []
        overlap_results = {
            (m1, m2): [] for m1 in search_methods for m2 in search_methods if m1 < m2
        }
        ...
        logger.info("\nPerformance Summary:")
        summary = (
            results_df.groupby(["Method", "TimeType"])["Time"]
            .agg(["mean", "std"])
            .reset_index()
        )
